...
class Skeletron_death:
    ...
    def search_skeletron(self):
        while keyboard.is_pressed('o') == False:

            try:

                # get the path/directory
                __folder_path = (r'D:\Summer_Programs_2021\Computerized_Vision_automation_tests\Bots\Terraria\skeletron_tracking')
                images = Path(__folder_path).glob('*.png')
                for __folder_path in images:
                    xb,yb = pyautogui.locateOnScreen(str(__folder_path), region=(6,389,2538,972), grayscale = False, confidence=.4)
...
__start_zombie_kill = Skeletron_death()
__loop_input = input("Type 'die' to begin")
while __loop_input == "die":
    __start_zombie_kill.search_skeletron()
